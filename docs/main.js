...
function initCardImageGenerator() {
    ...
    var normalColorFactorLists = [
		...
		["Victory", [0.65, 0.9, 0.5]],
		["Reaction", [0.5, 0.7, 1]],
		["Duration", [0.95, 0.55, 0.3]],
		["Reserve", [0.85, 0.75, 0.55]],
		["Curse", [0.75, 0.55, 0.95, -0.2, -0.2, -0.2, 1.0, 1.0, 1.0, 1.2, 0.8, 0.5]],
		["Shelter", [1, 0.55, 0.45]],
		["Ruins", [0.5, 0.35, 0.23]],
		["Landmark", [0.7, 1, 0.7, 0, 0, 0, 1, 1, 0.95, 1.05, 0.95, 0.85]],
		["Night", [0.35, 0.35, 0.35]],
		["Boon", [1.35, 1.2, 0.5, 0, 0, 0, 2.2, 1.65, 0.6, 1.85, 1.5, 0.65]],
		["Hex", [0.75, 0.65, 0.95, 0, 0, 0, 1, 0.9, 0.9, 1.0, 0.85, 1.1]],
		["State", [1.1, 1.25, 1.28, 0.5, 0.05, -0.1, 2.1, 1.8, 1.6, 1.3, 0.7, 0.45]],
		["Artifact", [1.21, 0.91, 0.67, 0, 0, 0, 1.08, 1, 1, 1.08, 1, 1]],
		["Project", [1.3, 0.85, 0.85, 0, 0, 0, 1.08, 1, 1, 1.08, 1, 1]],
		["Way", [0.9, 1.2, 1.35, 0, 0.1, 0.1, 1.45, 1.4, 1.4, 1.4, 1.3, 1.3]],
		["Ally", [1.1, 1.05, 0.95, 0.35, 0.3, 0.15, 1.15, 1.1, 1.05, 1.1, 1, 1]],
		["Trait", [1.1, 1.1, 1.3, 0, 0, 0, 1.4, 1.3, 1.3, 1.4, 1.3, 1.3]],
		["Prophecy", [0.1, 0.95, 1.1, -0.2, 0.1, 0.1, 1.15, 1.1, 1.1, 0.43, 0.43, 0.35]],
		["Ring", [1, 0.7, 0.8]]];
                ...
                while (word) {
                    ...
                    if (match) {
                        ...
                        if (words.length === 1 && !word.startsWith('+')) {
                            localY += 115 - scale * 48;
                            context.font = "900 192pt " + family;
                            localScale = 1.6;
                            if (templateSize === 3) {
                                context.font = "900 22pt " + family;
                        ...
                        if (match[3]) { //text in front of image
                            ...
                            if (cost.length >= 2) {
                                ...
                                if (specialCost != null) {
                                    cost = cost.slice(0, -1) + " ";
                                    context.font = "900 " + specialCostSize + "pt " + family;
                            ...
                            context.font = "900 " + 115 * bigNumberScale + "pt " + family;
                        ...
                        if (word.match(boldLinePatternWords) || word.match(boldLinePatternWordsSpecial)) {
                            if (words.length === 1)
                                context.font = "900 " + boldSize + "pt " + family;
                            else
                                context.font = "900 " + context.font;
                        }
                        if (context.font.includes('900')) {
                            ...
                            if (lastChar != "") {
                                ...
                                context.font = context.font.replace('900 ', '');
                ...
                for (var i = 0; i < words.length; ++i) {
                    ...
                    if (word === "\n") {
                        ...
                        if (line === "") //multiple newlines in a row
                            ...
                            context.font = "900 " + boldSize + "pt myText"; //resizing up to 64
...
//BELOW CHANGES THE TYPES LINE AMOUNT:

            if (typeLine.split(" - ").length >= 4 || typeLine.split(" - ").toString().length >= 26) {
            ...
            writeIllustrationCredit(181, 1272, "black", "900 ");
            writeCreatorCredit(1969, 1272, "black", "900 ");
    ...
    var sources = [
		...
		"TraitBrownSide.png",
	   "CardColorTwoColor.png" //32 REMOVE
...
class FontHandler {
    ...
    setFonts(lclFontTitle, lclFontSpecials, lclFontText) {
        ...
        if (lclFontText !== "") {
            ...
            css += this.getFontFaceCSS('myText', lclFontText, 'bold');
