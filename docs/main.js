...
// downloadPicture: always re-encode canvas to lossless PNG via UPNG.js
function downloadPicture() {
  function isTainted(ctx) {
    try {
      ...
      return (err.code === 18 || err.name === 'SecurityError' || err.name === 'DOMException');
    }
  }

  function rgbaToUint8Array(rgba) {
    return new Uint8Array(rgba.buffer ? rgba.buffer.slice(0) : new Uint8Array(rgba).buffer);
  }

  // Determine canvas id (keeps your existing id mapping logic)
  var id;
  if (templateSize == 0 || templateSize == 2 || templateSize == 3) {
    id = 0;
  } else if (templateSize == 1 || templateSize == 4) {
  ...
  // Build filename
  var title = (document.getElementById("title") || {value:''}).value.trim();
  var creator = (document.getElementById("creator") || {value:''}).value.trim();
  var fileName = (title.length > 0 ? title : "card");
  if (creator.length > 0) fileName += "_" + creator.split(" ")[0];
  fileName = fileName.split(" ").join("_") + ".png";

  // Read pixel data
  var w = canvas.width, h = canvas.height;
  var imgData = ctx.getImageData(0, 0, w, h);
  var rgba = imgData.data; // Uint8ClampedArray

  // Encode using UPNG (lossless)
  try {
    var arr = rgbaToUint8Array(rgba);
    var pngBuf = UPNG.encode([arr.buffer], w, h, 0); // 0 = lossless
    var blob = new Blob([pngBuf], { type: 'image/png' });
    var url = (window.webkitURL || window.URL).createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', fileName);
    setTimeout(function () { URL.revokeObjectURL(url); }, 60 * 1000);
  } catch (err) {
    console.error('PNG encode error:', err);
    // Fallback to native toDataURL
    try {
      var fallback = canvas.toDataURL('image/png').replace("image/png", "image/octet-stream");
      var url2 = (window.webkitURL || window.URL).createObjectURL(dataURLtoBlob(fallback));
      link.setAttribute('href', url2);
      link.setAttribute('download', fileName);
      setTimeout(function () { URL.revokeObjectURL(url2); }, 60 * 1000);
    } catch (e) {
